...
function getRules() {
  const sourceMap = {sourceMap: !PRODUCTION};
  const styleLoader = {loader: "style-loader", options: sourceMap};
  const cssLoader = {
    loader: "css-loader",
    options: {...sourceMap, minimize: PRODUCTION, importLoaders: true},
  };
  const postCssLoader = {
  ...
  if (PRODUCTION) {
    plugins = [
      ...
      new CopyWebpackPlugin([
        {from: path.join(SRC_PATH, "icons"), to: path.join(DIST_PATH, "icons")},
      ]),
    ];
  }
